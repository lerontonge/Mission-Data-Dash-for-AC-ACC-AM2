// if you made it this far, i would like you to know that i hijacked this code from Lovely Dashboard.
//Shout out to Constantinos Demetriadis for all his hard work, i just needed to borrow some of his functions.why reinvent the wheel?
//so credit where due.Thank you.

var mddu_blue = '#FF2B98FB';
var white = '#FFFFFFFF';
var black = '#FF000000';
var gray = '#FF444444';
...
// Settings File
const json_settings = readtextfile('./JavascriptExtensions/MDDU_settings.json');
const settings = JSON.parse(JSON.stringify(JSON.parse(json_settings)));

// Track Data
const json_tracks = readtextfile(
  './DashTemplates/' + folderName + '/JavascriptExtensions/MDDU_tracks.json'
);
const ld_tracks = JSON.parse(JSON.stringify(JSON.parse(json_tracks)));

// // Lovely Theme Colors
const json_colors = readtextfile(
  './DashTemplates/' + folderName + '/JavascriptExtensions/MDDU_colors.json'
);
const ld_colors = JSON.parse(JSON.stringify(JSON.parse(json_colors)));

function ld_getTheme() {
  const trueDarkMode =
    !settings || typeof settings.trueDarkMode === 'undefined'
      ? 1
      : parseInt(settings.trueDarkMode);
  return trueDarkMode;
}
...
function ld_GetRelDriverName(relPosition) {
  ...
  if ($prop('PersistantTrackerPlugin.Driver' + relPosition + '_Name')) {
    const name = $prop(
      'PersistantTrackerPlugin.Driver' + relPosition + '_Name'
    );
    return ld_formatName(name, driverName);
  } else {
    return '';
...
function ld_GetRelBestLapTime(relPosition) {
  if (
    timespantoseconds(
      $prop('PersistantTrackerPlugin.Driver' + relPosition + '_BestLapTime')
    ) == 0
  ) {
    return '--:--.---';
  } else {
    return $prop(
      'PersistantTrackerPlugin.Driver' + relPosition + '_BestLapTime'
    );
  }
}
...
function ld_GetRelLastLapTime(relPosition) {
  if (
    timespantoseconds(
      $prop('PersistantTrackerPlugin.Driver' + relPosition + '_LastLapTime')
    ) == 0
  ) {
    return '--:--.---';
  } else {
    return $prop(
      'PersistantTrackerPlugin.Driver' + relPosition + '_LastLapTime'
    );
  }
}
...
function ld_GetRelBestColor(relPosition) {
  if (
    driverdeltatobest(
      $prop('PersistantTrackerPlugin.Driver' + relPosition + '_Position')
    ) == 0
  ) {
    return ld_theme('ld_uiFastest');
  } else {
...
function ld_GetRelDriverColor(relPosition) {
  // Data expected is 'Ahead_01', 'Behind_00'
  // -> PersistantTrackerPlugin.DriverAhead_01_Name
  const driver_gap = drivergaptoplayer(
    $prop('PersistantTrackerPlugin.Driver' + relPosition + '_Position')
  );
  const driver_lap = timespantoseconds(
    driverlastlap($prop('DataCorePlugin.GameData.BestLapOpponentPosition') + 1)
  );
  if (
    driver_lap > 0 &&
    $prop('DataCorePlugin.GameData.SessionTypeName') == 'RACE'
  ) {
    if (
      $prop('PersistantTrackerPlugin.Driver' + relPosition + '_CurrentLap') -
        $prop('DataCorePlugin.GameRawData.Graphics.CompletedLaps') >=
      2
    ) {
...
function ld_GetRelCarLogo(relPosition) {
  ...
  let carName;
  !$prop('PersistantTrackerPlugin.Driver' + relPosition + '_CarName')
    ? (carName = 'none')
    : (carName = $prop(
        'PersistantTrackerPlugin.Driver' + relPosition + '_CarName'
      ));
  const carBrand = carName.split(' ');
  let color = '';
  const driver_gap = drivergaptoplayer(
  ...
  const driver_lap = timespantoseconds(
    driverlastlap($prop('DataCorePlugin.GameData.BestLapOpponentPosition'))
  );
  if (
    driver_lap > 0 &&
    $prop('DataCorePlugin.GameData.SessionTypeName') == 'Race'
    ...
    if (
      $prop('PersistantTrackerPlugin.Driver' + relPosition + '_CurrentLap') -
        $prop('DataCorePlugin.GameData.CurrentLap') >=
      2
    ) {
...
function ld_formatName(name, mode) {
  if (name != null) {
    ...
    if (full_name.length > 1) {
      // Check name for more than two parts (firstname & lastname)
...
function ld_getAvgValue(getAvgForProp, lapAvg, resetKey) {
  ...
  if (
    !$prop('DataCorePlugin.GamePaused') &&
    !$prop('DataCorePlugin.GameData.IsInPitLane') &&
    !$prop('DataCorePlugin.GameRawData.Graphics.IsSetupMenuVisible') &&
    prop != 0
  ) {
    if (
      $prop('DataCorePlugin.GameData.CurrentLap') - root['currentLap'] <=
      lapAvg
    ) {
      root['prop'].push(prop);
    } else {
  ...
  return Number.isNaN(ld_getAverage(root['prop']))
    ? '0'
    : ld_getAverage(root['prop']);
}
...
function ld_tireSlipLock(tyre) {
  ...
  if ($prop('DataCorePlugin.CurrentGame') == 'Automobilista2') {
    wheelSlip =
      $prop('DataCorePlugin.GameRawData.mTyreGrip' + tyre) < 0.1 ? 2 : 0;
    wheelSpeed = $prop(
      'DataCorePlugin.GameRawData.Physics.WheelAngularSpeed' + tyre
    );
    ...
    if ($prop('GameRawData.PlayerMotionData.m_wheelSlip03') * 100 > 25) {
      ...
      $prop('GameRawData.PlayerMotionData.m_wheelSlip03') * 100 <
      -50
    ) {
...
function ld_getEstimatedLapTime() {
  if (
    timespantoseconds(
      $prop('PersistantTrackerPlugin.EstimatedLapTime_SessionBestBased')
    ) != 0
  ) {
    return $prop('PersistantTrackerPlugin.EstimatedLapTime_SessionBestBased');
  } else if (
    timespantoseconds(
      $prop('PersistantTrackerPlugin.EstimatedLapTime_AllTimeBestBased')
    ) != 0
  ) {
...
function ld_getEstimatedDelta() {
  if (
    ...
    return ld_formatTime(
      timespantoseconds(
        $prop('PersistantTrackerPlugin.EstimatedLapTime_SessionBestBased')
      ) - timespantoseconds($prop('DataCorePlugin.GameData.BestLapTime'))
    );
  } else if (
    timespantoseconds(
      $prop('PersistantTrackerPlugin.EstimatedLapTime_AllTimeBestBased')
    ...
    return ld_formatTime(
      timespantoseconds(
        $prop('PersistantTrackerPlugin.EstimatedLapTime_AllTimeBestBased')
      ) - timespantoseconds($prop('PersistantTrackerPlugin.AllTimeBest'))
    );
  } else {
...
function ld_getEstimatedColour() {
  if (
    ...
    var timeDiffOverall =
      ...
      timespantoseconds(
        driverbestlap(
          $prop('DataCorePlugin.GameData.BestLapOpponentPosition') + 1
        )
      );
  } else if (
    timespantoseconds(
...
function ld_isLapInvalid() {
  ...
  return $prop('DataCorePlugin.GameRawData.Graphics.isValidLap') == 0 ||
    NewRawData().Cars[pos].CurrentLap.IsInvalid
    ? true
    : false;
}
...
function ld_getPreviousColour() {
  if ($prop('DataCorePlugin.CurrentGame') == 'IRacing') {
    var timeDiffMine =
      timespantoseconds($prop('DataCorePlugin.GameData.LastLapTime')) -
      timespantoseconds($prop('DataCorePlugin.GameData.BestLapTime'));
    var timeDiffOverall =
      timespantoseconds($prop('DataCorePlugin.GameData.LastLapTime')) -
      timespantoseconds(
...
function ld_sectorSegmentColor(sector) {
  var timeDiff =
    timespantoseconds(currentlapgetsectortime(sector, false)) -
...
function ld_sectorLastSegmentColor(sector) {
  var timeDiff =
    timespantoseconds(lastlapgetsectortime(sector, false)) -
    timespantoseconds(bestsectortime(sector, false));
  var timeDiffOverall =
    timespantoseconds(lastlapgetsectortime(sector, false)) -
    timespantoseconds(getbestsplittime(sector));
...
function ld_driverSectorSegmentColor(driver, sector) {
  var timeDiff =
    timespantoseconds(driversectorcurrentlap(driver, sector, false)) -
...
function ld_changed(delay, value) {
  root['ld_time'] =
    Math.floor(
      $prop('DataCorePlugin.CustomExpression.CurrentDateTime').getTime()
    ) / 1000;
  root['ld_oldstate'] =
    root['ld_oldstate'] == null ? value : root['ld_newstate'];
  root['ld_newstate'] = value;
  if (root['ld_newstate'] != root['ld_oldstate']) {
    root['ld_triggerTime'] = root['ld_time'];
  }
  return root['ld_triggerTime'] == null
    ? false
    : root['ld_time'] - root['ld_triggerTime'] <= delay / 1000;
}

function ld_isQuali() {
  if ($prop('DataCorePlugin.CurrentGame') == 'IRacing') {
    return $prop('DataCorePlugin.GameData.SessionTypeName') == 'Open Qualify' ||
    ...
    return $prop('GameRawData.PacketSessionData.m_sessionType') < 10
    ...
    return ucase($prop('DataCorePlugin.GameData.SessionTypeName')) ==
      'QUALIFY' ||
...
function ld_getTurn() {
  ...
  let currentPosition = drivertrackpositionpercent(
    getplayerleaderboardposition()
  ).toFixed(3);
  let margin = 0.01;
  for (const turn in trackTurns) {
    if (
      currentPosition >= trackTurns[turn] - margin &&
      currentPosition <= trackTurns[turn] + margin
    ) {
      return turn;
    }
...
function ld_boardScrollClass(boardHeight) {
  ...
  // boardHeight = Number of rows
  let maxScroll =
    $prop('DataCorePlugin.GameData.PlayerClassOpponentsCount') - boardHeight;
  let currentPos = $prop(
    'DataCorePlugin.GameRawData.Telemetry.PlayerCarClassPosition'
  );
  let midPos = Math.round(boardHeight / 2 + 3);
  let maxPos = Math.round(boardHeight / 2);
  // maxScroll can't be smaller than 3 (P3)
...
function ld_nightMode() {
  const nightMode =
    !settings || typeof settings.nightMode === 'undefined'
      ? 1
      : parseInt(settings.nightMode);
  if (nightMode === 1) {
    if ($prop('DataCorePlugin.CurrentGame') == 'AssettoCorsaCompetizione') {
      // Trigger Night Mode when lights on
      return $prop('DataCorePlugin.GameRawData.Graphics.LightsStage') > 0
        ? true
        : false;
    } else if ($prop('DataCorePlugin.CurrentGame') == 'Automobilista2') {
      // Trigger Night Mode when lights on
...
function ld_trueDarkMode() {
  const trueDarkMode =
    !settings || typeof settings.trueDarkMode === 'undefined'
      ? 1
      : parseInt(settings.trueDarkMode);
  const trueDarkModeNative =
    !settings || typeof settings.trueDarkModeNative === 'undefined'
      ? 0
      : parseInt(settings.trueDarkModeNative);
  if (
  ...
  if (trueDarkMode == 0) {
    ...
    if ($prop('DataCorePlugin.CurrentGame') == 'AssettoCorsaCompetizione') {
      ...
      if (trueDarkModeNative === 1) {
        return $prop('DataCorePlugin.GameRawData.Graphics.LightsStage') > 1
          ? true
          : false;
      } else {
...
function ld_analytics(screen) {
  const analytics =
    !settings || typeof settings.analytics === 'undefined'
      ? 0
      : parseInt(settings.analytics);
  if (analytics) {
